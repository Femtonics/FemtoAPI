...
% Last Modified by GUIDE v2.5 28-Nov-2017 10:25:13
...
% handles.image = readChannelData(handles.femtoapiObj, (unit.channels(1).handle)', 'uint16', ...
%     [[1 unit.dimensions(1).size] [1 unit.dimensions(2).size] [1 1] ] );
% 
...
handles = guidata(hObject);
%lutstruct = handles.lutstruct(handles.index);
%lutObj = LUT(lutstruct);
%colormap(lutObj.m_colorMap);
...
%sliderMin = [handles.lutstruct(handles.index).entries.value];
%sliderMin = sliderMin(1)+0.01;
sliderMin = handles.lutObjects(handles.index).m_vecBounds(1);
sliderMin = sliderMin + 0.01;
%sliderMax = [handles.lutstruct(handles.index).range];
%sliderMax = sliderMax(end);
sliderMax = handles.lutObjects(handles.index).m_rangeUpperBound;

%sliderValue = [handles.lutstruct(handles.index).entries.value];
%sliderValue = sliderValue(end);
...
%handles.femtoapiObj.readTaskParams;
...
s = {};
%.measurementSessions.measurements.measurementType; 
...
% sliderValue = fix(get(handles.slider1,'Value'));
% set(handles.FrameNumText,'String',num2str(sliderValue,'%d'));
% %fprintf('slider value: %f\n',get(handles.slider1,'Value'))
% 
% handles.frameNum = fix(get(hObject,'Value'));
