function [ colorAsArray ] = hex2array(hex,range)
% hex2rgb converts hex color values to rgb arrays on the range 0 to 1. 
%  The alpha value can be given optionally in the hex array. 
% 
% * * * * * * * * * * * * * * * * * * * * 
% SYNTAX:
...
switch range
    case 1
        colorAsArray = reshape(sscanf(hex.','%2x'),reshapeToDim,[]).'/255;

    case {255,256}
        colorAsArray = reshape(sscanf(hex.','%2x'),reshapeToDim,[]).';
